  ...
  return (
    <div
      className={`flex flex-col items-center rounded-2xl p-5 text-center transition-colors ${
        earned ? "bg-canvas" : "bg-fog opacity-60"
      }`}
    >
      {/* Icon */}
      <div
      ...
      >
        {earned ? (
          ...
          <div className="relative">
            <Icon className="h-7 w-7 text-slate" />
            <LockIcon className="absolute -bottom-0.5 -right-0.5 h-3.5 w-3.5 text-slate" />
          </div>
        )}
      </div>

      {/* Name — crisp title */}
      <p className="mt-3 font-polysans text-15 font-medium tracking-[-0.02em] text-graphite">
        {name}
      </p>

      {/* State badge */}
      {earned ? (
        <span className="mt-2 inline-flex items-center rounded-full bg-ember/10 px-2.5 py-0.5 font-polysans text-11 font-medium tracking-[-0.02em] text-ember">
          EARNED
        </span>
      ) : (
        <span className="mt-2 inline-flex items-center rounded-full bg-mist px-2.5 py-0.5 font-polysans text-11 font-medium tracking-[-0.02em] text-slate">
          LOCKED
        </span>
      )}
    </div>
